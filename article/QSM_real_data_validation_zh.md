...
這個結論很重要，因為在一般結構設計與地震工程中，加速度與反應譜一直是主要語言。它們可以提供設計尺度，也能讓不同地震波在同一個工程座標中比較。但是，當研究對象變成球面滑動隔震系統，問題就變得更細。隔震系統的核心反應是相對位移，而位移不是瞬間加速度峰值的直接結果。地震波若要把隔震層推開，必須透過速度脈衝、時間持續性、介面摩擦與系統有效週期共同作用。

因此，本文的立場很清楚，就是沿著原論文已經打開的門繼續走。原論文已經證明「只看加速度不夠」。本文接著問：如果只看加速度不夠，那麼能不能找到一條更接近位移生成機制的資料閱讀路徑？
...
本文以球面滑動隔震系統的真實實驗資料，對量子結構力學進行第一次實證性驗證。這項工作承接原研究最重要的發現：只看加速度或平均反應譜加速度，無法充分解釋隔震位移。本文沿著這個實證結果往前推進，嘗試說明地震波如何透過速度、能勢流、介面做功與頻率群，最後顯化為隔震位移。

第一個結論是，原研究的資料確實支持「只看加速度不夠」這件事。跨案例群組 Group1–4 的總結圖顯示，輸入加速度峰值 (Input Acceleration Peak) 與量測隔震位移峰值 (Measured IsoDisp Peak) 的相關性極低。當分析轉向 Input Velocity Peak，關係明顯提高。這說明隔震位移與速度脈衝、推動能力與時間持續性有關。對隔震系統來說，地震波需要在一段時間內推動隔震層滑移，單一加速度峰值無法完整描述這個過程。

第二個結論是，QSM 的視角轉換具有資料合理性。本文將 $a(t)$ 與 $v(t)$ 合成為 $P_{\mathrm{in}}(t)=a(t)v(t)$ ，用來描述輸入端量子結構力學能勢流。這個轉換讓地震輸入從單純的運動歷時，進一步變成可觀察的能勢流。當這個能勢流被整理為 Power Packet，並進一步進入隔震介面形成 $F_{\mathrm{interface}}(t)v_{\mathrm{rel}}(t)$ 的 Interface Power / Work Exchange 時，它與位移反應的關係最清楚。這表示隔震位移的形成，必須同時看輸入端能勢、介面交換與實際位移反應。

第三個結論是，隔震位移背後的頻率結構，不能只用單一有效頻率概括。 $f_{\mathrm{eff}}$ 很重要，它提供了隔震系統的入口尺度。實際資料顯示，位移反應還包含 QSM Power Frequency Group、Interface Frequency Group 與 Displacement Response Frequency Group 的連續轉換。各組 Figure 2 提供了分組證據：Group1 顯示短週期脈衝如何以時間事件方式接觸有效頻率族；Group2 顯示中週期脈衝如何透過較強能勢波包與倍頻入口形成大位移；Group3 顯示非脈衝地震波仍可能以分散寬頻能勢場造成大位移；Group4 顯示頻譜匹配資料仍保留不同的能勢路徑。

單筆診斷圖中的頻率群收斂比較，是本文最重要的驗證證據之一。許多高位移案例的位移反應頻率並非無規則散布，而是靠近原本系統有效頻率及其倍頻所形成的整齊序列。這代表隔震系統的有效頻率可以理解為一組能勢入口族。地震波的 QSM Power Frequency Group 進入系統後，會經由介面選擇與轉換，最後在 Displacement Response Frequency Group 中顯化。J 圖呈現的重點，是能勢如何沿著有效頻率族被接收、轉換，並形成可觀測的位移反應。

因此，本文對 QSM 的核心論述可以收束為一句話：

> 地震波對結構物而言，是一個通過系統的能勢場；當結構無法在這個能勢場中轉化為穩定的能量流動通道時，能勢會在介面形成做功交換，並顯化為隔震位移。

這也是本文對原實驗資料的主要貢獻。原研究已經指出，只看加速度不足以解釋隔震位移；本文進一步說明，速度、能勢流、介面做功與頻率群共同構成了更完整的資料閱讀路徑。QSM Power Flow 的價值，在於把地震輸入從 Input Motion 推進到 Power Packet，再推進到 Interface Exchange，最後連到 Frequency-Group Manifestation。這條路徑讓我們可以更清楚地描述地震能勢如何進入系統、穿過介面、接觸頻率群，並最後成為位移。
